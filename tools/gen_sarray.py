#!/usr/bin/env python3
"""Generate inc/array/sarray.h and src/array/sarray.c for escsafelib.

The four element families are identical modulo the element type, so they are
emitted from one template rather than typed four times by hand.
"""
...
# suffix, C type, signed, accumulator type, accumulator max/min, human name
TYPES = [
    ("u8",  "uint8_t",  False, "uint32_t", "0xFFFFFFFFu", None,        "unsigned 8 bit"),
    ("u16", "uint16_t", False, "uint32_t", "0xFFFFFFFFu", None,        "unsigned 16 bit"),
    ("u32", "uint32_t", False, "uint32_t", "0xFFFFFFFFu", None,        "unsigned 32 bit"),
    ("i32", "int32_t",  True,  "int32_t",  "INT32_MAX",   "INT32_MIN", "signed 32 bit"),
]
...
uint8_t sarrayGet${S} ( const ${T}* arr, uint32_t arrSize, uint32_t index, ${T}* value );
uint8_t sarraySet${S} ( ${T}* arr, uint32_t arrSize, uint32_t index, ${T} value );
uint8_t sarrayFill${S} ( ${T}* arr, uint32_t arrSize, ${T} value );
uint8_t sarrayClearSecure${S} ( ${T}* arr, uint32_t arrSize );
uint8_t sarrayCopy${S} ( ${T}* dest, uint32_t destSize, const ${T}* src, uint32_t srcSize );
uint8_t sarrayCopyN${S} ( ${T}* dest, uint32_t destSize, const ${T}* src, uint32_t srcSize, uint32_t count );
uint8_t sarrayMove${S} ( ${T}* dest, uint32_t destSize, const ${T}* src, uint32_t srcSize, uint32_t count );
uint8_t sarraySwap${S} ( ${T}* arr, uint32_t arrSize, uint32_t indexA, uint32_t indexB );
uint8_t sarrayCompare${S} ( const ${T}* a, uint32_t aSize, const ${T}* b, uint32_t bSize, int32_t* result );
uint8_t sarrayCompareN${S} ( const ${T}* a, uint32_t aSize, const ${T}* b, uint32_t bSize, uint32_t count, int32_t* result );
uint8_t sarrayFind${S} ( const ${T}* arr, uint32_t arrSize, ${T} value, uint32_t* index );
uint8_t sarrayFindLast${S} ( const ${T}* arr, uint32_t arrSize, ${T} value, uint32_t* index );
uint8_t sarrayCount${S} ( const ${T}* arr, uint32_t arrSize, ${T} value, uint32_t* count );
uint8_t sarrayIsSorted${S} ( const ${T}* arr, uint32_t arrSize, uint8_t* result );
uint8_t sarrayBinarySearch${S} ( const ${T}* arr, uint32_t arrSize, ${T} value, uint32_t* index );
uint8_t sarrayMin${S} ( const ${T}* arr, uint32_t arrSize, ${T}* value, uint32_t* index );
uint8_t sarrayMax${S} ( const ${T}* arr, uint32_t arrSize, ${T}* value, uint32_t* index );
uint8_t sarraySum${S} ( const ${T}* arr, uint32_t arrSize, ${ACC}* sum );
uint8_t sarrayReverse${S} ( ${T}* dest, uint32_t destSize, const ${T}* src, uint32_t srcSize );
uint8_t sarrayRotate${S} ( ${T}* arr, uint32_t arrSize, uint32_t shift );
uint8_t sarraySort${S} ( ${T}* arr, uint32_t arrSize );
uint8_t sarrayInsert${S} ( ${T}* arr, uint32_t arrSize, uint32_t* count, uint32_t index, ${T} value );
uint8_t sarrayRemove${S} ( ${T}* arr, uint32_t arrSize, uint32_t* count, uint32_t index, ${T}* removed );
""")
 ...
 */
static void reverseRange${S} ( ${T}* arr, uint32_t first, uint32_t last )
{
    ${T} temp = 0;
    uint32_t span = 0;
    uint32_t i = 0;
...
{
    uint8_t retVal = SA_OK;
    volatile ${T}* target = ( volatile ${T}* ) arr;
    uint32_t i = 0;
 ...
 * @note    Use sarrayMove${S} when the ranges may overlap.
 */
uint8_t sarrayCopyN${S} ( ${T}* dest, uint32_t destSize, const ${T}* src, uint32_t srcSize, uint32_t count )
{
    uint8_t retVal = SA_OK;
    ...
    else if ( spanBytes ( count, ( uint32_t ) sizeof ( ${T} ), &bytes ) == FALSE )
    {
        retVal = SA_INVALIDSIZE;
    }
 ...
 * @param[in]  src       Source array.
 * @param[in]  srcSize   Capacity of src in elements.
 * @return  SA_OK on success, otherwise the status sarrayCopyN${S} reports.
...
uint8_t sarrayCopy${S} ( ${T}* dest, uint32_t destSize, const ${T}* src, uint32_t srcSize )
{
    ...
    retVal = sarrayCopyN${S} ( dest, destSize, src, srcSize, srcSize );
 ...
 * @note    No match is not an error here, unlike sarrayFind${S}. The answer
 ...
 * @note    This is the precondition check for sarrayBinarySearch${S}.
 */
uint8_t sarrayIsSorted${S} ( const ${T}* arr, uint32_t arrSize, uint8_t* result )
{
    uint8_t retVal = SA_OK;
 ...
 *          sarrayIsSorted${S} first when the order cannot be guaranteed.
...
uint8_t sarrayReverse${S} ( ${T}* dest, uint32_t destSize, const ${T}* src, uint32_t srcSize )
{
    ...
    else if ( spanBytes ( srcSize, ( uint32_t ) sizeof ( ${T} ), &bytes ) == FALSE )
    {
        retVal = SA_INVALIDSIZE;
    }
    else if ( isOverlapping ( dest, bytes, src, bytes ) == TRUE )
...
uint8_t sarrayRotate${S} ( ${T}* arr, uint32_t arrSize, uint32_t shift )
{
    ...
    else
    {
        ...
        if ( k != 0 )
        {
            reverseRange${S} ( arr, 0, k - 1u );
            reverseRange${S} ( arr, k, arrSize - 1u );
            reverseRange${S} ( arr, 0, arrSize - 1u );
        }
        else
        {
            // Intentionally blank.
 ...
 *          Call sarrayClearSecure${S} on the whole buffer when the discarded
...
SUM_UNSIGNED = """            if ( ( ( uint32_t ) arr[ i ] ) > ( 0xFFFFFFFFu - accumulator ) )
            {
                retVal = SA_OVERFLOW;
                done = TRUE;
            }
...
SUM_SIGNED = """            if ( ( arr[ i ] > 0 ) && ( accumulator > ( INT32_MAX - arr[ i ] ) ) )
            {
                retVal = SA_OVERFLOW;
                done = TRUE;
            }
...
SUMNOTE_UNSIGNED = """ * @note    The total is a uint32_t whatever the element type is, so a
...
SUMNOTE_SIGNED = """ * @note    Both directions are checked. A negative element can drive the
...
def build_header():
    parts = [HEADER_HEAD]
    for suffix, ctype, signed, acc, accmax, accmin, human in TYPES:
        parts.append(PROTOS.substitute(S=suffix, T=ctype, ACC=acc, HUMAN=human))
...
def build_source():
    ...
    for suffix, ctype, signed, acc, accmax, accmin, human in TYPES:
        parts.append(FAMILY.substitute(
            ...
            ACC=acc,
            HUMAN=human,
            SUMBODY=(SUM_SIGNED if signed else SUM_UNSIGNED),
            SUMNOTE=(SUMNOTE_SIGNED if signed else SUMNOTE_UNSIGNED),
        ))
    return "".join(parts)


def main():
    os.makedirs(os.path.join(REPO, "inc", "array"), exist_ok=True)
    os.makedirs(os.path.join(REPO, "src", "array"), exist_ok=True)

    hpath = os.path.join(REPO, "inc", "array", "sarray.h")
    cpath = os.path.join(REPO, "src", "array", "sarray.c")
